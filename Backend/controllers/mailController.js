...
const sendConfirmationEmails = async (req, res) => {
  try {
    ...
    const managerMailOptions = {
      ...
      subject: `Appraisal Review Submission - ${user.empName}`,
  ...
  try {
    ...
    const HRMailOptions = {
      ...
      to:"naveenkumar44820@gmail.com" ,
       subject : `Appraisal reviewed by manager - ${user.managerName}`,
...
const sendGoalsAddedEmails = async (req, res) => {
  try {
    ...
    const userMailOptions = {
      from: process.env.EMAIL_USER,
      to: user.email,
      subject: `Goals assigned by ${user.managerName} for ${nextYear} - ${nextYear2} `,
      html: `
        <p>Dear ${user.empName},</p><p>This is a system-generated email to inform you that your manager <strong>${user.managerName}</strong> has assigned you goals on <strong>${currentDate}</strong> for the year <strong>${nextYear} - ${nextYear2}</strong>.</p>
  ...
  const sendFinalHREmails = async (req, res) => {
    try {
      ...
      const userMailOptions = {
        from: process.env.EMAIL_USER,
        to: user.email,
        subject: `Your appraisal has been reviewed for ${presentYear} - ${nextYear}`,
        html: `
