      ...
      if (response.ok) {
        ...
        const weightageResponse = await fetch(
          `http://localhost:3003/appraisal/overAllWeightage/${employeeId}/${timePeriod[0]}/${timePeriod[1]}`,
          {
            ...
            body: JSON.stringify({ overallWeightage: calculatedWeightage, performanceRating, areasOfGrowth})
          }
        );
      ...
      <div className="mb-6">
        {userData ? (
          ...
          <textarea
            ...
            className="w-3/4 p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 min-h-[100px] resize-none"
